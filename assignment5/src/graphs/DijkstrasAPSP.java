...
public class DijkstrasAPSP {
	...
	private static float[] D;										// Array containing the cost of shortest path that only goes through vertices in S
	private static int currentMinimum;								// Contains the index of the vertex which represents the minimum cost to get out of s
	...
	public static float[] dijkstra(AdjMatrixGraph g, int s) {
		...
		for (int n = 0; n <= vertices; n++) {
			int w = findMinimum(D, S);			// line 5 of algorithm
			if (w > 0) {
	...
	public static int findMinimum(float[] D, ArrayList<Integer> S) {
